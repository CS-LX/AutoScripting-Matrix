...
本mod提供了一系列实用的小功能，可大大提升玩家们的游玩体验以及操作感受~

### 调试辅助绘制（Gizmos）

本功能灵感来源于Unity引擎。在Unity引擎中，如果你选中了一个物体，物体会有亮眼的描边；如果场景中存在摄像机，则会显示摄像机的视锥，让开发者知晓这个摄像机可以看到哪些东西...这些便于调试的绘制图形都统称为**Gizmos**。

在本mod里，Gizmos功能与Unity的类似，都起到辅助调试、可视化查看电子元件功能的作用。你可以在一些方块的介绍中（如：[方块展示板](#方块展示板)、[复杂摄像机LED](#复杂摄像机LED)）找到此方块是否具有Gizmos功能，亦或是此方块的Gizmos的具体内容。

使用Gizmos控制器点击带有Gizmos功能的方块可以开启/关闭此方块的Gizmos显示。打开Gizmos控制器的编辑界面可以统筹管理所有方块的Gizmos。
...
| 矩阵平方开方器  | 将矩阵内所有元素的平方和再进行开方，最终输出。（即勾股定理，只不过有16个项） |
...
| 简单摄像机LED   | 输入：前后左右底端        | 详见[简单摄像机LED](#简单摄像机LED)                                                                                   |
...
| 传送门控制器     | 输入：前后左右端         | 详见[传送门控制器](#传送门控制器)                                                                                       |
...
- 坐标轴：【红、绿、蓝短线】表示展示方块绘制时采用的坐标系（即经过矩阵变换的坐标系）。

- 方位指示线：【粉→紫罗兰渐变色直线】由展示板坐标指向展示方块的位置。
...
- 坐标轴：【红、绿、蓝短线】表示摄像机的本地坐标系（即经过视角矩阵变换的坐标系）。

- 方位指示线：【粉→紫罗兰渐变色直线】由摄像机LED板指向摄像机的位置。

- 实体框：【黄色正方体线框】指示摄像机所在的位置。
...
- M12：屏幕UV细分。此值有两个作用：决定摄像机是否采用屏幕UV绘制画面、决定屏幕UV绘制画面的细分程度。
  
  摄像机采用屏幕UV绘制画面（M12>0：是，反之否）：即计算摄像机板显示屏四个角在屏幕的位置，显示出的画面的四个角不再是原始画面的四个角，而是根据屏幕UV映射过后的四个角。若要实现传送门效果，则详见[此视频3:10~3:40](https://www.bilibili.com/video/BV1Ht4y1e7Ri/)。
  
  屏幕UV绘制画面的细分程度：此值越小，细分程度越低，画面偏移越大，画面就越异常。所以为保证画面效果，此值建议设置为16~32。
...
- 视锥框：【橙色线框】显示摄像机的视锥，在视锥中的物体都会被摄像机拍摄到。

#### 传送门控制器

顾名思义，它可以根据你的输入矩阵创建或控制传送门。这个传送门是有画面的，你可以透过传送门看到传送门另一端的影像。

它会创造两个传送门，当你进入一个传送门后，它会将你传送至另一个传送门。

**输入**

| 端口  | 作用        | 详细                                                                                                |
| --- | --------- |:------------------------------------------------------------------------------------------------- |
| 前   | 传送门1的变换矩阵 | 默认的传送门门扉（即此输入为单位阵时），位置为位于方块中心、朝向为正上方的正方形（与复杂摄像机差不多）。<br/>此矩阵为这个正方形门扉的四个端点的变换。（无法显示画面则可能因为这个输入未到位） |
| 右   | 传送门1的控制矩阵 | 详见下方“**控制矩阵**”                                                                                    |
| 后   | 传送门2的变换矩阵 | 详见上方“**传送门1的变换矩阵**”                                                                               |
| 左   | 传送门2的控制矩阵 | 详见下方“**控制矩阵**”                                                                                    |

**控制矩阵**

- M11：控制传送门的状态
  
  - ⌊M11⌋=0：传送门**不**显示画面，**不**执行传送
  
  - ⌊M11⌋=1：传送门既显示画面，也执行传送
  
  - ⌊M11⌋=2：传送门仅显示画面，**不**执行传送
  
  - ⌊M11⌋=3：传送门**不**显示画面，仅执行传送

- M12：是否自动防止传送瞬间画面闪烁（M12>0：是，反之否）
  
  - 解释：当玩家靠传送门门扉足够近时，玩家摄像机近裁剪平面会与传送门门扉相交，进而暴露传送门门扉后的画面，即玩家会“看透”传送门。这会导致玩家靠近传送门传送时画面闪烁。
    
    为解决此问题，传送门门扉会依据特定算法，在玩家靠近时后退，保证不被玩家的摄像机“看透”。这也就避免了传送时闪烁的问题。**（但是玩家速度很快时仍会闪烁，例如玩家穿了数值被暴力修改的跑鞋）**
...
> **注意：控制器元件有输入两个控制矩阵，但是控制矩阵中有关设置传送门边框的部分，只由其中一个控制矩阵控制！请玩家注意甄别。（这是一个bug，涉及到底层逻辑，较难修复）**
