...
namespace Game {
    public class ASMSimCameraLEDBlock : ASMMountedElectricElementBlock {
        ...
        public override void Initialize() {
            ...
            texture = ContentManager.Get<Texture2D>("Textures/ASMSimCameraLED");
            for (int i = 0; i < 6; i++) {
        ...
        public override string MainDescription(int value) => "创建一个摄像机，并且将摄像机捕捉到的画面显示出来。\r\n摄像机视角矩阵（摄像机方位、旋转）为输入矩阵。";
        ...
        public override ASMElectricElement CreateElectricElement(SubsystemASMElectricity subsystemElectricity, int value, int x, int y, int z) {
            return new ASMSimCameraLEDElectricElement(subsystemElectricity, new CellFace(x, y, z, GetFace(value)));
        }
